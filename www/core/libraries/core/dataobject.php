<?php
    ...
    abstract class DataObject extends CMSObject {
        ...
        public function order_by() {
            $alias = self::getTableAlias($this->get_table_name());
            return "{$alias}.`id`";
        }
        ...
        protected function load_list_get_fields($params = array()) {
            ...
            foreach ($fields as $f) {
                $f = strtolower($f);
                if (false !== strpos($f, 'internal_') || 0 === strpos($f, "{$table_abr}._") ) {
                    continue;
                }
        ...
        protected function load_list_get_where($params) {
        	...
        	if ($this->hasField('is_hidden')) {
        		$table = $this->getTableName();
        		$alias = $this->getTableAlias($table);
        		$params['where'][] = "($alias.is_hidden IS NULL OR $alias.is_hidden=0)";
        	}
...
//echo '<pre>' . $sql."</pre>\n\n";die();
        ...
        public function load($id, $params = array()) {
            ...
            $params['where'][] = "$abr.id=$id";
            $params['group_by'] = "$abr.id";
