...
class TFileField extends TField {
   ...
   */
  function GetAsHTML() {
    $Res = sprintf("<input type=\"file\" name=\"%s\" size=\"%u\"".HtmlUtils::attributes($this->attributes).">",
      $this->Name,
      $this->Size);
    return $Res;
  }
...
class TEditField extends TField{
  ...
  function GetAsHTML($aSize = 0, $aMaxLength = 0){
    $Res = sprintf("<input type=\"%s\" name=\"%s\" size=\"%u\" maxlength=\"%u\" value=\"%s\" ".$this->attributes.">",
      $this->getInputType(),
      $this->Name,
      $aSize ? $aSize : $this->Size,
      $aMaxLength ? $aMaxLength : $this->MaxLength,
      $this->GetHTMLSafetyValue());
    return $Res;
  }
}
...
class TTextField extends TField{
  ...
  function GetAsHTML($aCols = 0, $aRows = 0) {
    ...
    return $res
           .HtmlUtils::attributes($this->attributes)
           .'>'
           .$this->GetHTMLsafetyValue()
           .'</textarea>';
  }
...
class TSelectOption{
  ...
  function GetAsHTML($aSelectedValue){
    return sprintf("<option value=\"%s\"%s>%s</option>",
      ...
      (string) $this->Value == (string) $aSelectedValue? " selected" : "",
...
class TRadioOption{
  ...
  function GetAsHTML($aName, $aSelectedValue, $aAlign = "left"){
    ...
    $Res .= sprintf("<input type=\"radio\" name=\"%s\" value=\"%s\"%s ".HtmlUtils::attributes($this->attributes).">",

      htmlspecialchars($aName),
      htmlspecialchars($this->Value),
      $this->Value == $aSelectedValue? " checked" : "");
    if ($this->Caption && $aAlign == "left") $Res .= "&nbsp;".$this->Caption;
...
class TRadioField extends TField{
  ...
  function TRadioField($aName, $aValue, $aValues, $attributes = NULL, $aSeparator = "<br>") {
    ...
    reset($aValues);
    while(list($key, $value) = each($aValues))
      $this->Options[count($this->Options)] = new TRadioOption($key, $value, $attributes);
  }
    ...
    if ($aDB && $aQuery){
      if ($result = mysql_query($aQuery, $aDB)){
        while($r = mysql_fetch_array($result))
          $this->Options[count($this->Options)] = new TRadioOption($r[0], $r[1]);
      }
...
class TDateField extends TField{
  ...
  function GetAsHTML(){
    ...
    $minYear = date('Y') + (!is_null($this->options) && array_key_exists('year.min', $this->options)
                            ? intval($this->options['year.min']) : 0);
    $maxYear = date('Y') + (!is_null($this->options) && array_key_exists('year.max', $this->options)
                            ? $this->options['year.max'] : 10);
