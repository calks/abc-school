	...
	class profileModule extends Module {
		...
		protected function taskTax_return_form($params=array()) {
			...
			if (Request::isPostMethod()) {
				...
				$attachments = $this->loadTaxReturnFormAttachments();
				...
				if (!$this->errors) {
		            ...
		            $smarty = Application::getSmarty();
		            $template_path = $this->getTemplatePath('tax_return_form_email');
		            $smarty->assign('form', $form);
		            ...
		            $msg->setSubject("abc-school.ru: заявка на обучение налогового вычета");
		            ...
		            if ($sent) {
		            	Redirector::redirect("/{$this->getName()}/tax_return_form?sent=1");
		            }
		            else {
			...
			$smarty->assign('form_action', Application::getSeoUrl("/{$this->getName()}/tax_return_form"));
		...
		protected function loadTaxReturnFormAttachments() {			
